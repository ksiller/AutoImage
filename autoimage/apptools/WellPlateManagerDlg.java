 ...
 */
public class WellPlateManagerDlg extends javax.swing.JDialog {

    private WellPlateConfig lastSavedConfig;
    private static String lastFileLocation=System.getProperty("user.home");

    
    public WellPlateManagerDlg(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    ...
    public void setWellplateLayout(WellPlateConfig configuration) {
        if (configuration!=null) {
            platenameField.setValue(configuration.getName());
            if (configuration.getFileLocation().equals("")) {
                configuration=configuration
                        .copy()
                        .fileLocation(lastFileLocation)
                        .build();
//                configuration.fileLocation=lastFileLocation;
            }
            fileLocationLabel.setText(configuration.getFileLocation());
            //convert all values from um to mm
            plateWidthField.setValue(configuration.getWidth()/1000);
            plateLengthField.setValue(configuration.getLength()/1000);
            plateHeightField.setValue(configuration.getHeight()/1000);
            columnsSpinner.setValue(configuration.getColumns());
            rowsSpinner.setValue(configuration.getRows());
            leftEdgeToA1Field.setValue(configuration.getDistLeftEdgeToA1Center()/1000);
            topEdgeToA1Field.setValue(configuration.getDistTopEdgeToA1Center()/1000);
            wellDiameterField.setValue(configuration.getWellDiameter()/1000);
            wellDistanceField.setValue(configuration.getWellToWellDistance()/1000);
            wellDepthField.setValue(configuration.getWellDepth()/1000);
            wellShapeComboBox.setSelectedItem(configuration.getWellShape());
            materialComboBox.setSelectedItem(configuration.getBottomMaterial());
            bottomThicknessField.setValue(configuration.getBottomThickness()/1000);
        }
        lastFileLocation=configuration.getFileLocation();
        lastSavedConfig=configuration;
    }
    
    private WellPlateConfig parseConfiguration() {
        //convert all dimensions from mm to um
        WellPlateConfig configuration=new WellPlateConfig.Builder()
            .name((String)platenameField.getValue())
            .fileLocation(fileLocationLabel.getText())
            .width(1000*((Number)plateWidthField.getValue()).doubleValue())
            .length(1000*((Number)plateLengthField.getValue()).doubleValue())
            .height(1000*((Number)plateHeightField.getValue()).doubleValue())
            .columns(((Number)columnsSpinner.getValue()).intValue())
            .rows(((Number)rowsSpinner.getValue()).intValue())
            .distLeftEdgeToA1Center(1000*((Number)leftEdgeToA1Field.getValue()).doubleValue())
            .distTopEdgeToA1Center(1000*((Number)topEdgeToA1Field.getValue()).doubleValue())
            .wellDiameter(1000*((Number)wellDiameterField.getValue()).doubleValue())
            .wellToWellDistance(1000*((Number)wellDistanceField.getValue()).doubleValue())
            .wellDepth(1000*((Number)wellDepthField.getValue()).doubleValue())
            .wellShape((String)wellShapeComboBox.getSelectedItem())
            .bottomMaterial((String)materialComboBox.getSelectedItem())
            .bottomThickness(1000*((Number)bottomThicknessField.getValue()).doubleValue())
            .build();
    ...
    private void closeWindow() {
        ...
        if (!lastSavedConfig.equals(parseConfiguration())) {
    ...
    private void saveLayout() {
        WellPlateConfig config = parseConfiguration();
        if ((config.getWidth() < config.getDistLeftEdgeToA1Center()+(config.getColumns()-1)*config.getWellToWellDistance()+config.getWellDiameter()/2)
                || (config.getLength() < config.getDistTopEdgeToA1Center()+(config.getRows()-1)*config.getWellToWellDistance()+config.getWellDiameter()/2)) {
            JOptionPane.showMessageDialog(this,"Cannot fit all wells into layout.");
            return;
        }
        if (config.getWellDiameter() <= 0) {
            JOptionPane.showMessageDialog(this,"Well diameter has to be larger than 0");
            return;
        }
        if (config.getWellToWellDistance() < config.getWellDiameter()) {
            JOptionPane.showMessageDialog(this,"Well-to-well distance has to be larger than the well diameter.");
            return;
        }
        //initialize with file location and name in current configuration
//        IAcqLayout acqLayout= new WellPlateLayout(new File(config.getFileLocation(),config.getName()),config);
        JFileChooser jfc = new JFileChooser();
        if (config.getFileLocation()==null || "".equals(config.getFileLocation())) {
            //set file location to user's home directory
            config=config
                    .copy()
                    .fileLocation(System.getProperty("user.home"))
                    .build();
        }
        jfc.setCurrentDirectory(new File(config.getFileLocation()));
//        String fname=acqLayout.getFile().getName();
        String fname=config.getName();
...
//        jfc.ensureFileIsVisible(acqLayout.getFile());
        jfc.ensureFileIsVisible(new File(fname));
        ...
        do {
            ...
            if (selectedFile.exists()) {
                ...
                int overwrite = JOptionPane.showConfirmDialog(null, "File \"" + selectedFile.getName()+"\" already exists.\nReplace?", "", JOptionPane.YES_NO_OPTION);
                if (overwrite == JOptionPane.YES_OPTION) {
                    continueToSave=true;
                }
            }
        } while (!continueToSave);    
        IAcqLayout acqLayout= new WellPlateLayout(selectedFile,config);
        acqLayout.setName(selectedFile.getName());
//        acqLayout.setFile(selectedFile);
        ...
        try {
            ...
            try {
                ...
                Logger.getLogger(WellPlateManagerDlg.class.getName()).log(Level.SEVERE, null, ex);
    ...
    private void openButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_openButtonActionPerformed
        //check if modified and if yes, open save dialog
        WellPlateConfig currentConfig=parseConfiguration();
        WellPlateConfig startupIgnoreFile=lastSavedConfig
                .copy()
                .fileLocation(currentConfig.getFileLocation())
                .name(currentConfig.getName())
                .build();
        if (!startupIgnoreFile.equals(currentConfig)) {
            int result=JOptionPane.showConfirmDialog(this, "Save current plate configuration?", "Plate Layout Manager", JOptionPane.YES_NO_CANCEL_OPTION);
            switch (result) {
                case JOptionPane.CANCEL_OPTION: {
                    return;
        ...
        if (returnVal == JFileChooser.APPROVE_OPTION) {
            ...
            if (!(layout instanceof WellPlateLayout)) {
                JOptionPane.showMessageDialog(this, "Layout file '"+f.getName()+"' is not a well plate format.");
                return;
            }
            IJ.log("layout.getFile: "+layout.getFile().getAbsolutePath());
            WellPlateConfig configuration=new WellPlateConfig.Builder()
                .name(layout.getName())
                .fileLocation(layout.getFile().getParentFile().getAbsolutePath())
                .width(layout.getWidth())
                .length(layout.getLength())
                .height(layout.getHeight())
                .columns(((WellPlateLayout)layout).getColumns())
                .rows(((WellPlateLayout)layout).getRows())
                .distLeftEdgeToA1Center(((WellPlateLayout)layout).getLeftEdgeToA1())
                .distTopEdgeToA1Center(((WellPlateLayout)layout).getTopEdgeToA1())
                .wellDiameter(((WellPlateLayout)layout).getWellDiameter())
                .wellToWellDistance(((WellPlateLayout)layout).getWellToWellDistance())
                .wellDepth((((WellPlateLayout)layout).getWellDepth()))
                .wellShape(((WellPlateLayout)layout).getWellShape())
                .bottomMaterial(layout.getBottomMaterial())
                .bottomThickness(layout.getBottomThickness())
                .build();
            setWellplateLayout(configuration);
    ...
    private void newButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_newButtonActionPerformed
        ...
        WellPlateConfig config=new WellPlateConfig.Builder()
                .fileLocation(lastFileLocation)
                .build();
        setWellplateLayout(config);
