...
import autoimage.gui.models.AreaTableModel;
import autoimage.gui.models.AFTableModel;
import autoimage.gui.models.ChannelTableModel;
...
import autoimage.events.IStageMonitorListener;
import autoimage.data.layout.Landmark;
import autoimage.data.layout.AbstractLayout;
import autoimage.data.acquisition.SequenceConfig;
import autoimage.data.layout.CustomLayout;
import autoimage.data.layout.WellPlateLayout;
import autoimage.data.Detector;
import autoimage.services.AcqJobManager;
import autoimage.data.acquisition.AcquisitionJob;
import autoimage.data.FieldOfView;
import autoimage.events.IMergeAreaListener;
import autoimage.events.IRefPointListener;
import autoimage.services.JobCallback;
import autoimage.services.JobException;
import autoimage.services.LiveModeMonitor;
import autoimage.utils.MMCoreUtils;
import autoimage.data.layout.WellPlateConfig;
import autoimage.services.StagePosMonitor;
import autoimage.data.layout.TilingSetting;
import autoimage.utils.Utils;
import autoimage.data.Vec3d;
import autoimage.data.layout.area.PolygonArea;
import autoimage.data.layout.area.RectArea;
import autoimage.data.layout.area.CompoundArea;
import autoimage.data.layout.area.EllipseArea;
import autoimage.apptools.WellPlateManagerDlg;
import autoimage.apptools.LayoutManagerDlg;
import autoimage.apptools.ZOffsetDlg;
 ...
 */
public class AcqFrame extends javax.swing.JFrame implements MMListenerInterface, ActionListener, TableModelListener, WindowListener, ILiveListener, IMergeAreaListener {//, IDataProcessorListener {
...
//    private boolean isCalculatingTiles;
    ...
    private static final int LAYOUT_MODE_MOVETO = 1;
    private static final int LAYOUT_MODE_SELECT = 2;
    private static final int LAYOUT_MODE_ZOOM = 4;
    private static final int LAYOUT_MODE_COMMENT = 8;
    private static final int LAYOUT_MODE_NEW_RECTANGLE = 16;
    private static final int LAYOUT_MODE_NEW_ELLIPSE = 32;
    private static final int LAYOUT_MODE_NEW_POLYGON = 64;
    ...
    public void stagePositionChanged(final StagePositionChangedEvent e) {
        SwingUtilities.invokeLater(new Runnable() {
            ...
            @Override
            public void run() {
                Double[] stagePos = new Double[] {e.getStageX(),e.getStageY(),e.getStageZ()};
                ...
                if (stagePos[0]!=null && stagePos[1]!=null) {
                    BasicArea area = acqLayout.getFirstContainingAreaAbs(stagePos[0], stagePos[1], currentAcqSetting.getTileWidth_UM(), currentAcqSetting.getTileHeight_UM());
                    if (area != mostRecentArea) {
                        if (area != null) {
                            areaLabel.setText("Area: "+area.getName());
//                            a.setAcquiring(true);
                        } else {
...
//                stageMonitor.removeListener(zOffsetDialog);
                stageMonitor.unregisterForEvents(zOffsetDialog);
            }
            zOffsetDialog=null;
        }
    }
...
//        Landmark.setCameraRot(cameraAngle);
...
//                stageMonitor.addListener(zOffsetDialog);
                stageMonitor.registerForEvents(zOffsetDialog);
            }
...
//        zOffsetDialog.updateConfigData(currentAcqSetting.getChannelGroupStr(),zOffsetDialog.convertChannelList(currentAcqSetting.getChannels()));
    ...
    private void showManagePlateLayout(boolean modal) {
        WellPlateConfig plateConfig=new WellPlateConfig.Builder()
                .fileLocation("")
                .build();
        WellPlateManagerDlg plateDialog=new WellPlateManagerDlg(this, modal);
        plateDialog.setWellplateLayout(plateConfig);
    ...
    private void showManageCustomLayout(boolean modal) {
        ...
        if (!acqLayout.isEmpty() && !(acqLayout instanceof WellPlateLayout)) {
            try {
                //pass copy of layout
                layoutDialog.setCustomLayout(AbstractLayout.createFromJSONObject(acqLayout.toJSONObject(),acqLayout.getFile()));
            } catch (JSONException ex) {
                layoutDialog.setCustomLayout(new CustomLayout());
            }
        } else {
            layoutDialog.setCustomLayout(new CustomLayout());
        }
    ...
    public void landmarksUpdated(final LandmarkListEvent e) {
        SwingUtilities.invokeLater(new Runnable() {
            ...
            public void run() {
                ...
                for (Landmark r:e.getRefAreaList()) {
                    if (r.isStagePosMapped()) {
                        mapped=true;
                        break;
                    }
                }
                setLandmarkFound(mapped);
                calcTilePositions(null, currentAcqSetting, ADJUSTING_SETTINGS);        
                acqLayoutPanel.repaint();
            }
        });
...
*/
    ...
    public AcqFrame(ScriptInterface gui, EventBus evtbus) {
        app = gui;
        eventBus = evtbus;
        ...
        eventBus.register(acqLayoutPanel);
        
        ButtonGroup navigationButtonGroup =  new ButtonGroup();
        navigationButtonGroup.add(zoomButton);
        navigationButtonGroup.add(selectButton);
        navigationButtonGroup.add(moveToButton);
        navigationButtonGroup.add(commentButton);
        navigationButtonGroup.add(newRectangleButton);
        navigationButtonGroup.add(newEllipseButton);
        navigationButtonGroup.add(newPolygonButton);
        navigationButtonGroup.add(mergeAreasButton);
        ...
        //refresh every 100ms
        stageMonitor = new StagePosMonitor(core,100, eventBus);
//        stageMonitor.addListener(this);
//        stageMonitor.addListener((LayoutPanel)acqLayoutPanel);
        stageMonitor.registerForEvents(this);
        stageMonitor.registerForEvents(acqLayoutPanel);
...
*/
    ...
    private void initComponents() {
        ...
        mergeAreasButton.setToolTipText("Merge Areas in Layout");
        ...
        moveToButton.setFont(new java.awt.Font("Lucida Grande", 0, 11)); // NOI18N
        moveToButton.setText("Move To");
        moveToButton.setToolTipText("Click to move to layout position");
        moveToButton.setSize(new java.awt.Dimension(70, 29));
                    ...
                    .addGroup(statusPanelLayout.createSequentialGroup()
                        .addGroup(statusPanelLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            ...
                            .addGroup(statusPanelLayout.createSequentialGroup()
                                ...
                                .addGap(0, 0, 0)
                                .addComponent(moveToButton, javax.swing.GroupLayout.PREFERRED_SIZE, 77, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addGap(0, 0, 0)
        ...
        statusPanelLayout.linkSize(javax.swing.SwingConstants.VERTICAL, new java.awt.Component[] {commentButton, moveToButton, selectButton, setLandmarkButton, stageControlButton, zoomButton});
...
/*
    private String runAcquisition(final AcqSetting setting) {
        ...
        if (!initializeChannelTable(currentAcqSetting)) {
            return null;
        }
        
            final Timer timer=new Timer();
            acquisitionTask = new AcquisitionTask(app, imageDestPath,currentAcqSetting, acqLayout,this, sequenceDir);
                            ...
                            if (acquisitionTask.hasStarted()) {
                                ...
                                if (!app.getHideMDADisplayOption()) {
                                    try {
                                        ...
                                        virtualDisplay = new VirtualAcquisitionDisplay(acquisitionTask.getImageCache(), acquisitionTask.getPrefix());
                                        acquisitionTask.getImageCache().addImageCacheListener(virtualDisplay);
                                        virtualDisplay.show();
                                    } catch (MMScriptException ex) {
                                        virtualDisplay=null;
                                        Logger.getLogger(AcqFrame.class.getName()).log(Level.SEVERE, null, ex);
...
*/
    ...
    public String startAcquisition() {
        ...
        try {
            for (AcqSetting setting:acqSettings) {           
                final SequenceConfig config = new SequenceConfig(setting, acqLayout, null);

                Runnable preInit=new JobCallback<SequenceConfig>(config) {
                    @Override
                    public void execute(SequenceConfig config) throws RuntimeException {
                        IJ.log("CALLBACK PRE INIT: "+config.getAcqSetting().getName());
                    }
                };
                
                Runnable postInit=new JobCallback<SequenceConfig>(config) {
                    @Override
                    public void execute(SequenceConfig config) throws RuntimeException {
                ...
                Runnable preRun=new JobCallback<SequenceConfig>(config) {
                    @Override
                    public void execute(final SequenceConfig config) throws RuntimeException {
                ...
                Runnable postRun=new JobCallback<SequenceConfig>(config) {
                    @Override
                    public void execute(SequenceConfig config) throws RuntimeException {
...
//                    if ((newZoom != oldZoom)) {
    ...
    private void acqLayoutPanelMousePressed(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_acqLayoutPanelMousePressed
        ...
        if (layoutMode==LAYOUT_MODE_SELECT 
                || layoutMode==LAYOUT_MODE_COMMENT 
                || layoutMode==LAYOUT_MODE_MERGE_AREAS 
                || layoutMode==LAYOUT_MODE_NEW_RECTANGLE 
                || layoutMode==LAYOUT_MODE_NEW_ELLIPSE) {
            if (selPath==null) {
...
//                    IJ.log("Right MousePressed");  
        ...
        } else if (layoutMode==LAYOUT_MODE_NEW_POLYGON) {    
    ...
    private void acqLayoutPanelMouseReleased(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_acqLayoutPanelMouseReleased
        if (layoutMode==LAYOUT_MODE_NEW_RECTANGLE || layoutMode==LAYOUT_MODE_NEW_ELLIPSE) {
            if (selPath!=null) {
                Point2D endPoint=((LayoutPanel) acqLayoutPanel).convertPixToLayoutCoord(new Point(evt.getX(), evt.getY()));
                Rectangle2D rect=selPath.getPath().getBounds2D();
//                Rectangle2D rect=createRectangle2D(selPath.getStart(), endPoint);
                BasicArea area=null;
                if (layoutMode==LAYOUT_MODE_NEW_RECTANGLE) {
                    area=new RectArea(
                        createNewAreaName(), 
                        acqLayout.createUniqueAreaId(), 
                        rect.getCenterX(), 
                        ...
                        false, 
                        "");
                } else if (layoutMode==LAYOUT_MODE_NEW_ELLIPSE) {
            ...
            selPath=null;
        } else if (selPath!=null && (layoutMode==LAYOUT_MODE_SELECT || layoutMode==LAYOUT_MODE_MERGE_AREAS || layoutMode==LAYOUT_MODE_COMMENT)) {
            if (selPath.getPath()!=null) {
                ...
                if (!identicalPoints(startPoint, endPoint)) { 
                    ...
                    } else if (layoutMode==LAYOUT_MODE_MERGE_AREAS || layoutMode==LAYOUT_MODE_COMMENT) {
                ...
                AreaTableModel atm = (AreaTableModel) areaTable.getModel();
                if (layoutMode==LAYOUT_MODE_SELECT && al!=null) {
                    retilingAllowed = false;
                ...
                } else if (layoutMode==LAYOUT_MODE_MERGE_AREAS) {
                    for (BasicArea area : al) {
                ...
                } else if (layoutMode==LAYOUT_MODE_COMMENT && isLeftMouseButton && al.size() > 0) {
...
//                    commentButton.requestFocus();
    ...
    private void acqLayoutPanelMouseDragged(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_acqLayoutPanelMouseDragged
        ...
        ((LayoutPanel) acqLayoutPanel).updateSelectionPath(selPath);
        if (layoutMode==LAYOUT_MODE_NEW_POLYGON && selPath.getPath()!=null) {
            selPath.getPath().lineTo(coordX, coordY);
        } else if (layoutMode==LAYOUT_MODE_SELECT || layoutMode==LAYOUT_MODE_MERGE_AREAS || layoutMode==LAYOUT_MODE_COMMENT) {
        ...
        } else if (layoutMode==LAYOUT_MODE_NEW_RECTANGLE) {
            if (evt.isControlDown()) {
                double w=Math.abs(coordX-selPath.getStart().getX());
                double h=Math.abs(coordY-selPath.getStart().getY());
                selPath.setPath(new Path2D.Double(new Rectangle2D.Double(
                    selPath.getStart().getX()-w,
                    selPath.getStart().getY()-h,
                    w * 2,
                    h * 2)));
            } else {
                selPath.setPath(new Path2D.Double(new Rectangle2D.Double(
            ...
            Rectangle2D pathBounds=selPath.getPath().getBounds2D();
            selPath.getPath().append(Utils.createLinePath(
                    pathBounds.getCenterX()-5, 
                    pathBounds.getCenterY(), 
                    pathBounds.getCenterX()+5, 
                    pathBounds.getCenterY()), false);
            selPath.getPath().append(Utils.createLinePath(
                    pathBounds.getCenterX(), 
                    pathBounds.getCenterY()-5,
                    pathBounds.getCenterX(), 
                    pathBounds.getCenterY()+5), false);
        } else if (layoutMode==LAYOUT_MODE_NEW_ELLIPSE) {
            if (evt.isControlDown()) {
                double w=Math.abs(coordX-selPath.getStart().getX());
                double h=Math.abs(coordY-selPath.getStart().getY());
        ...
        if (acqManager.hasActiveJobs()) {
            ...
            if (engine==null || !engine.isRunning()) {
                ...
                if (refPointListDialog != null) {
                    if (stageMonitor!=null) {
//                        stageMonitor.removeListener(refPointListDialog);
                        stageMonitor.unregisterForEvents(refPointListDialog);
                    }
                    refPointListDialog.dispose();
                    refPointListDialog=null;
                }
...
//                    if (!isWaiting) {
                    if (acqEng2010.isRunning() && isAcquiring) {
                        acqEng2010.stop();
                        //will automatically lead to call imagingFinished
                    } else {
                        if (acquisitionTask!=null) {
                            acquisitionTask.cancel();
                        } else {
...
//                        Landmark.setCameraRot(fieldRot);
                        acqLayoutPanel.repaint();
//                        break;
                    }
                    if (result ==  JOptionPane.YES_OPTION) {
...
//                }
        ...
        if (!acqEng2010.isRunning() && isAcquiring) {
            //problem in syncing GUI and acqEng2010 status, should never happen
...
//        fc.setCurrentDirectory(new File(acqLayout.getFile().getPath()));
        ...
        if (returnVal == JFileChooser.APPROVE_OPTION) {
            ...
            IAcqLayout layout=AbstractLayout.loadLayout(f);
            if (layout==null) {
                ...
                Landmark.Builder.setStageToLayoutRot(0);
                ...
                mergeAreasButton.setToolTipText(!acqLayout.isEmpty() && acqLayout.isAreaMergingAllowed() ? "Merge Areas in Layout" : "Areas cannot be merged in this layout");
...
//                stageMonitor.addListener(refPointListDialog);
                ...
                JLabel cornerLabel;
                if (acqLayout instanceof WellPlateLayout) {
                    ...
                    WellPlateLayout plate=(WellPlateLayout)acqLayout;
                    ((AcqWellplateRule)layoutColumnHeader).setOffset(plate.getLeftEdgeToA1());
                    ((AcqWellplateRule)layoutColumnHeader).setWellDistance(plate.getWellToWellDistance());
                    ...
                    ((AcqWellplateRule)layoutRowHeader).setWellDistance(plate.getWellToWellDistance());
...
//                areaTable.repaint();
    ...
    private void updateTileOverlap() {
        ...
        double overlap = 0;
        try {
            overlap = (double) Integer.parseInt(tileOverlapField.getText()) / 100;
        } catch (NumberFormatException nfe) {
        }
        if (overlap != currentAcqSetting.getTileOverlap() && retilingAllowed && acqLayout.getTilingStatus()!=IAcqLayout.TILING_IN_PROGRESS) {//isCalculatingTiles) {
            prevTilingSetting = currentAcqSetting.getTilingSetting().copy().build();
            prevObjLabel = currentAcqSetting.getObjective();
            currentAcqSetting.setTileOverlap(overlap);
            calcTilePositions(null, currentAcqSetting, ADJUSTING_SETTINGS);
        }
...
/*        DataProcessor dp;
    ...
    private void loadProcTreeButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_loadProcTreeButtonActionPerformed
        ...
        if (fc.getSelectedFile()!=null) {
            ...
            try {
                ...
                //cannot parse JSONObject, try to load is as experiment settings fileLocation
                try {
    ...
    private void commentButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_commentButtonActionPerformed
        boolean commentMode = commentButton.isSelected();
        layoutMode = commentMode ? LAYOUT_MODE_COMMENT : 0;
        if (commentMode) {
            acqLayoutPanel.setCursor(normCursor);
        }
    ...
    private void selectButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_selectButtonActionPerformed
        boolean selectMode = selectButton.isSelected();
        layoutMode=selectMode ? LAYOUT_MODE_SELECT : 0;
        if (selectMode) {
            acqLayoutPanel.setCursor(normCursor);
        }
    ...
    private void zoomButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_zoomButtonActionPerformed
        boolean zoomMode = zoomButton.isSelected();
        layoutMode=zoomMode ? LAYOUT_MODE_ZOOM : 0;
        if (zoomMode) {
    ...
    private void moveToButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_moveToButtonActionPerformed
        if (moveToButton.isSelected() && acqLayout.getNoOfMappedStagePos() == 0) {
            JOptionPane.showMessageDialog(this,"At least one layout landmark needs to be set first.");
            moveToButton.setSelected(false);
            return;
        }
        boolean moveToMode = moveToButton.isSelected();
        layoutMode=moveToMode ? LAYOUT_MODE_MOVETO : 0;
        if (moveToMode) {
...
//            refPointListDialog.addListener(this);
            ...
            refPointListDialog.setAcqLayout(acqLayout);
        }
                ...
                public void run() {
                    try {
                        ...
                        for (int row : rows) {
                            ...
                            core.setPosition(core.getFocusDevice(), startZPos+ch.getZOffset());
...
//                app.getAutofocusManager().refresh();
    ...
    private void newPolygonButtonItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_newPolygonButtonItemStateChanged
        boolean newPolygonMode=newPolygonButton.isSelected();
        layoutMode=newPolygonMode ? LAYOUT_MODE_NEW_POLYGON : 0;
        ((LayoutPanel)acqLayoutPanel).updateSelectionPath(selPath);
        selPath=null;
    ...
    private void newRectangleButtonItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_newRectangleButtonItemStateChanged
        boolean newRectangleMode=newRectangleButton.isSelected();
        layoutMode=newRectangleMode ? LAYOUT_MODE_NEW_RECTANGLE : 0;
        ((LayoutPanel)acqLayoutPanel).updateSelectionPath(selPath);
        selPath=null;
    ...
    private void newEllipseButtonItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_newEllipseButtonItemStateChanged
        boolean newEllipseMode=newEllipseButton.isSelected();
        layoutMode=newEllipseMode ? LAYOUT_MODE_NEW_ELLIPSE : 0;
        ((LayoutPanel)acqLayoutPanel).updateSelectionPath(selPath);
        selPath=null;
...
//            layoutFile = f;
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            @Override
            public void run() {
                new AcqFrame(null,null).setVisible(true); //need to check null as parameter
            }
        });
    }
    ...
    private void loadPreferences() {
        expSettingsFile  = new File(Prefs.get("edu.virginia.autoimage.expSettingsFile", System.getProperty("user.home")));
        layoutPath = Prefs.get("edu.virginia.autoimage.layoutPath", System.getProperty("user.home"));
        dataProcessorPath=Prefs.get("edu.virginia.autoimage.dataProcessorPath", System.getProperty("user.home"));
        imageDestPath = Prefs.get("edu.virginia.autoimage.rootDir", System.getProperty("user.home"));
...
/*        if (!fileLocation.exists()) {
            IJ.log("Loading experiment settings: "+fileLocation.getAbsolutePath() + " not found, creating default settings.");
            expMessage="- Experiment fileLocation not found."; 
            error=true;
        } else {   */
            ...
            try {
                ...
                JSONObject expSettingsObj=new JSONObject(sb.toString());
                try {
                    layout=AbstractLayout.createFromJSONObject(expSettingsObj.getJSONObject(IAcqLayout.TAG_LAYOUT), file);
                } catch (JSONException ex) {
                    IJ.log("LOADING EXPERIMENT SETTING ERROR:"+ex.toString());
                    error=true;
                }
                try {
                    settings=loadAcquisitionSettings(expSettingsObj.getJSONArray(AcqSetting.TAG_ACQ_SETTING_ARRAY));  
...
//        }
        ...
        if (settings!=null || layout!=null) {
            ...
            expSettingsFile=new File(System.getProperty("user.home"),"not found");
            expSettingsFileLabel.setText(expSettingsFile.getName());
        ...
        if (layout==null) {
            if (revertToDefault) {
                ...
                layout=new CustomLayout();//creates emptyLayout
                //set active layout
                acqLayout=layout;
            } else {
...
//            calcTilePositions(null, currentAcqSetting.getFieldOfView(), currentAcqSetting.getTilingSetting(), ADJUSTING_SETTINGS);
            ...
            JLabel cornerLabel = new JLabel(acqLayout instanceof WellPlateLayout ? "Well":"mm");
            cornerLabel.setFont(new Font("Arial", Font.PLAIN, 12));
            layoutScrollPane.setCorner(JScrollPane.UPPER_LEFT_CORNER, cornerLabel);
            layoutScrollPane.setColumnHeaderView(layoutColumnHeader);
            layoutScrollPane.setRowHeaderView(layoutRowHeader);
...
//                if (!core.deviceBusy(xyStageName))
    ...
    private void moveToLandmark(int index) {
        Landmark lm = acqLayout.getLandmark(index);
        if (lm != null) {
            try {
                String zStageName = core.getFocusDevice();
                core.waitForDevice(zStageName);
