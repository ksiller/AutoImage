...
/*        Toolkit toolkit = Toolkit.getDefaultToolkit();
    ...
    @Subscribe
    public void landmarkSelectionChanged(LandmarkSelectionChangedEvent e) {
        if (e.getLayout()==acqLayout) {
            if (selectedLandmarks.isEmpty() && (e.getSelectedRefAreas()==null || e.getSelectedRefAreas().isEmpty())) {
                //save a repaint
                return;
            }
            selectedLandmarks.clear();
            if (e.getSelectedRefAreas()!=null) {
                for (Landmark selR:e.getSelectedRefAreas()) {
                    selectedLandmarks.put(selR.getSerialNo(), selR);
                }
            }
            repaint();
        }
...
//                calculateScale(w,h);
                ...
                for (int i=0; i<areas.size(); i++) {
                    ...
                    if (a.isSelectedForAcq() && a.getTilePositions()!=null) {
                        for (Tile t : a.getTilePositions()) {
                            AffineTransform affine=AffineTransform.getTranslateInstance(t.getCenterXPos(), t.getCenterYPos());
    ...
    private void drawLandmarks(Graphics2D g2d) {
        if ((acqLayout!=null) && (acqLayout.getLandmarks()!=null)) {
            List<Landmark> landmarks=acqLayout.getLandmarks();
            for (Landmark lm:landmarks) {
                boolean selected=selectedLandmarks.containsKey(lm.getSerialNo());
//                boolean selected=selectedLandmarks.containsValue(lm);
                g2d.setColor(selected ? COLOR_SELECTED_LANDMARK : COLOR_LANDMARK);
                g2d.setStroke(lm.isStagePosMapped() ? SOLID_STROKE : DASHED_STROKE);
...
//        g2d.draw(fullChip);
    ...
    public boolean setCurrentXYStagePos(double xPos, double yPos) {
        boolean stageMoved=currentStagePos_X!=xPos || currentStagePos_Y!=yPos;
...
//        Graphics2D g2d = (Graphics2D)g2d.create();
        ...
        //ensure that BasicArea and Landmark work with current layout rotation value
        BasicArea.setStageToLayoutRot(acqLayout.getStageToLayoutRot());
        Landmark.Builder.setStageToLayoutRot(acqLayout.getStageToLayoutRot());
